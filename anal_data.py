...
#정렬순서가 최근 데이터가 맨 뒤에
def generateData(source_data,timeslot): # 시계열 훈련데이터 생성
    x_data = [] ; y_data =[]
...
def confirm_data(x_data,y_data,source_datas): #문제데이터와 정답데이터의 일치성 확인
    result_bool=True
    if y_data[0]!= x_data[1][-1]:
        result_bool=False
    if y_data[1]!= x_data[2][-1]:
        result_bool=False
    if y_data[-1]!= source_datas[-1].astype("float"): # 마지막 데이터 확인
        result_bool=False
    if y_data[-2]!= source_datas[-2].astype("float"):
        result_bool=False
    return result_bool #True일때 일치

def scatterAnal(x_data,y_data,weights,title): #산점도
    # 가중평균 적용
